# ============================================================================
# SuSiE-R Analysis for NHANES Data - Table S1 and S2 Variables Combined
# ============================================================================
# This script:
# 1. Loads prepared data
# 2. Filters to Table S1 (Health Model) and Table S2 (Nutrition Model) variables
...
output_dir <- file.path(base_dir, "susie_results_table_s1_s2")
...
# ============================================================================
# Define Table S1 and S2 Variables
# ============================================================================
...
# Combine both tables
combined_vars <- c(table_s1_vars, table_s2_vars)

cat("============================================================================\n")
cat("SuSiE-R Analysis for NHANES Data - Table S1 and S2 Variables Combined\n")
cat("============================================================================\n")
cat("Method:", SUSIE_METHOD, "\n")
cat("Table S1 variables:", length(table_s1_vars), "\n")
cat("Table S2 variables:", length(table_s2_vars), "\n")
cat("Total combined variables:", length(combined_vars), "\n")
cat("============================================================================\n\n")
...
# ============================================================================
# Step 1: Load Data and Filter to Table S1 and S2 Variables
# ============================================================================

cat("Step 1: Loading data and filtering to Table S1 and S2 variables...\n")

# Load the full dataset
full_data <- readRDS(file.path(base_dir, "nhanes_filtered_for_susie.rds"))

# Get combined variable names (NHANES codes)
combined_nhanes_vars <- unlist(combined_vars)

# Find variables that exist in the dataset
available_vars <- intersect(combined_nhanes_vars, names(full_data))
missing_vars <- setdiff(combined_nhanes_vars, names(full_data))

cat("  Available combined variables:", length(available_vars), "/", length(combined_nhanes_vars), "\n")
if (length(missing_vars) > 0) {
  cat("  Missing variables:", paste(missing_vars, collapse = ", "), "\n")
}

# Identify which variables are from S1 vs S2
s1_nhanes_vars <- unlist(table_s1_vars)
s2_nhanes_vars <- unlist(table_s2_vars)
available_s1_vars <- intersect(s1_nhanes_vars, available_vars)
available_s2_vars <- intersect(s2_nhanes_vars, available_vars)

cat("  Available Table S1 variables:", length(available_s1_vars), "/", length(table_s1_vars), "\n")
cat("  Available Table S2 variables:", length(available_s2_vars), "/", length(table_s2_vars), "\n")
...
# Create dataset with only combined variables and outcome
data_combined <- full_data %>%
  select(SEQN, all_of(available_vars), all_of(outcome_var))

cat("  Final dataset dimensions:", nrow(data_combined), "rows x", ncol(data_combined), "columns\n")
cat("  Combined variables included:", length(available_vars), "\n\n")
...
cat("  Complete cases for outcome:", nrow(data_combined), "\n")

# Separate predictors and outcome
X_data <- data_combined %>%
  select(-SEQN, -all_of(outcome_var))

y_data <- data_combined[[outcome_var]]
...
# ============================================================================
optimal_params_file <- file.path(base_dir, "susie_parameter_optimization_table_s1_s2", "optimal_parameters.csv")
...
if (exists("z_scores") && length(z_scores) > 0) {
  ...
  p_z <- ggplot(z_df, aes(x = Index, y = Z_Score)) +
    ...
    labs(
      title = "Z-Scores from Univariate Regressions - Table S1 & S2 Combined",
...
p_pip <- ggplot(pip_df, aes(x = Index, y = PIP)) +
  ...
  scale_color_manual(values = c("TRUE" = "red", "FALSE" = "black"),
                     ...
                     name = "") +
  labs(
    title = "Posterior Inclusion Probabilities (PIP) - Table S1 & S2 Combined",
...
if (length(fit$sets$cs) > 0) {
  ...
  p_cs_size <- ggplot(cs_summary, aes(x = CS, y = Size)) +
    ...
    labs(
      title = "Credible Sets Summary - Table S1 & S2 Combined",
  ...
  p_cs_purity <- ggplot(cs_purity_long, aes(x = CS, y = Value, fill = Purity_Metric)) +
    ...
    scale_fill_brewer(palette = "Set2", 
                      ...
                      name = "Purity Metric") +
    labs(
      title = "Credible Sets Purity Metrics - Table S1 & S2 Combined",
  ...
  if (length(cs_comparison_list) > 0) {
    ...
    if (nrow(cs_comparison_df) > 0) {
      p_comp <- ggplot(cs_comparison_df, aes(x = abs(Z_Score), y = PIP)) +
        ...
        labs(
          title = "PIP vs Z-Score Comparison within Credible Sets - Table S1 & S2",
...
p_roc <- ggplot() +
  ...
  labs(
    title = "ROC Curves: Training vs Test - Table S1 & S2 Combined",
...
p_var_imp <- ggplot(coef_df, aes(x = reorder(Variable, AbsCoefficient), y = AbsCoefficient)) +
  ...
  coord_flip() +
  labs(
    title = "Top 30 Variables by Absolute Coefficient (SuSiE-R) - Table S1 & S2 Combined",
    x = "Variable",
    y = "|Coefficient|"
  ) +
...
if (length(selected_vars) > 0 && length(selected_vars) < 100) {
  ...
  p_shap_summary <- ggplot(shap_df, aes(x = reorder(Variable, Abs_SHAP), y = SHAP_Value)) +
    ...
    scale_fill_manual(values = c("TRUE" = "red", "FALSE" = "blue"), 
                     ...
                     name = "Effect") +
    labs(
      title = "SHAP-like Values: Top 20 Variables - Table S1 & S2 Combined",
  ...
  p_shap_waterfall <- ggplot(example_shap, aes(x = reorder(Variable, Contribution), y = Contribution)) +
    ...
    scale_fill_manual(values = c("TRUE" = "red", "FALSE" = "blue"),
                     name = "Direction") +
    labs(
      title = "SHAP Waterfall Plot (Example Prediction) - Table S1 & S2 Combined",
...
latex_code <- paste0(
  "% LaTeX code for SuSiE-R results - Table S1 & S2 Combined Variables\n",
  "% Generated automatically by run_susie_table_s1_s2.R\n",
  "% Reference: https://stephenslab.github.io/susieR/articles/finemapping.html\n\n",
  "\\begin{table}[htbp]\n",
  "  \\centering\n",
  "  \\input{susie_results_table_s1_s2/performance_metrics_table.tex}\n",
  "\\end{table}\n\n",
  "% SuSiE-R Fine-mapping Plots (similar to susieR example)\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/z_scores_plot.pdf}\n",
  "  \\caption{Z-scores from univariate regressions - Table S1 & S2 Combined. ",
  "Red dashed lines indicate |z| = 2.}\n",
  "  \\label{fig:z_scores_s1_s2}\n",
  "\\end{figure}\n\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/pip_plot.pdf}\n",
  "  \\caption{Posterior Inclusion Probabilities (PIP) for all variables - Table S1 & S2 Combined. ",
  "Variables in credible sets are highlighted in red.}\n",
  "  \\label{fig:pip_plot_s1_s2}\n",
  "\\end{figure}\n\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/credible_sets_size.pdf}\n",
  "  \\caption{Number of variables in each credible set identified by SuSiE-R - Table S1 & S2 Combined.}\n",
  "  \\label{fig:credible_sets_size_s1_s2}\n",
  "\\end{figure}\n\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/credible_sets_purity.pdf}\n",
  "  \\caption{Purity metrics for each credible set - Table S1 & S2 Combined.}\n",
  "  \\label{fig:credible_sets_purity_s1_s2}\n",
  "\\end{figure}\n\n",
  "% Performance Evaluation Plots\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/roc_curves.pdf}\n",
  "  \\caption{ROC curves comparing training and test performance - Table S1 & S2 Combined. ",
  "Training AUC = ", round(metrics_train$AUC, 3), 
  ", Test AUC = ", round(metrics_test$AUC, 3), ".}\n",
  "  \\label{fig:roc_curves_s1_s2}\n",
  "\\end{figure}\n\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.9\\textwidth]{susie_results_table_s1_s2/confusion_matrices.pdf}\n",
  "  \\caption{Confusion matrices for training (left) and test (right) sets - Table S1 & S2 Combined.}\n",
  "  \\label{fig:confusion_matrices_s1_s2}\n",
  "\\end{figure}\n\n",
  "\\begin{figure}[htbp]\n",
  "  \\centering\n",
  "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/variable_importance.pdf}\n",
  "  \\caption{Top 30 variables selected by SuSiE-R ranked by absolute coefficient magnitude - Table S1 & S2 Combined.}\n",
  "  \\label{fig:variable_importance_s1_s2}\n",
  "\\end{figure}\n"
)
...
if (file.exists(file.path(output_dir, "shap_summary.pdf"))) {
  latex_code <- paste0(latex_code,
    "\\begin{figure}[htbp]\n",
    "  \\centering\n",
    "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/shap_summary.pdf}\n",
    "  \\caption{SHAP-like values for top 20 variables - Table S1 & S2 Combined.}\n",
    "  \\label{fig:shap_summary_s1_s2}\n",
    "\\end{figure}\n\n",
    "\\begin{figure}[htbp]\n",
    "  \\centering\n",
    "  \\includegraphics[width=0.8\\textwidth]{susie_results_table_s1_s2/shap_waterfall.pdf}\n",
    "  \\caption{SHAP waterfall plot - Table S1 & S2 Combined.}\n",
    "  \\label{fig:shap_waterfall_s1_s2}\n",
    "\\end{figure}\n"
  )
}
...
cat("============================================================================\n")
cat("SuSiE-R ANALYSIS COMPLETE - TABLE S1 & S2 VARIABLES COMBINED\n")
cat("============================================================================\n")
